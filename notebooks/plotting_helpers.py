...
def plot_subnet_training(
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        if type == "mask":
            ...
            ax.axvspan(training_start, training_end, color="red", alpha=0.2)
        if mask_lines:
...
def plot_full_training(
    run_id, metric="acc", mask_lines=True, skip_pretraining_step=True
):
    METRICS = ("acc", "loss")
    if metric not in METRICS:
        raise ValueError(f"metric must be one of {METRICS}")
...
def plot_subnet_boxplots(
    ...
    rand_seq_cols = [
        [f"test_{metric}_random_{i}" for i in range(n)] for n in n_seq_random_subnets
    ]
    ...
    rand_seq_values = [
        [ss[c] for c in rsc] for rsc, ss in zip(rand_seq_cols, summary_seq)
    ]
    rand_std_values = [summary_std[c] for c in rand_std_cols]
    rand_no_values = [summary_no[c] for c in rand_no_cols]

    fig, axes = plt.subplots(
        1, 2, figsize=figsize, sharey=True, width_ratios=width_ratios
    )
    ...
    bp = axes[0].boxplot(all_values, whis=(0, 100), patch_artist=True)
    ...
    axes[1].bar(
        range(len(bars)),
        bars,
        color=[f"C{i}" for i in range(len(bars))],
        width=0.5,
        align="center",
    )
