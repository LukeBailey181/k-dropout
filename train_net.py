...
def get_dataset(
    ...
    if dataset_name == 'mnist':
        ...
        train_loader, test_loader = get_cifar10(batch_size, test_batch_size, num_workers)
    else:
        raise ValueError(f'Unknown dataset {dataset_name}')

    if preprocess:
        return process_dataset(train_loader, device), process_dataset(test_loader, device)
...
if __name__ == '__main__':
    ...
    # model
    parser.add_argument(
            '--dropout_layer', type=str, choices=DROPOUT_LAYERS, required=True,
            help='Type of dropout layer to use')
    parser.add_argument(
            '--input_size', type=int, required=True,
            help='Dimension of the model input')
    parser.add_argument(
            '--hidden_size', type=int, required=True,
            help='Dimension of the model hidden layer(s)')
    parser.add_argument(
            '--n_hidden', type=int, required=True,
            help='Number of hidden layers')
    parser.add_argument(
            '--output_size', type=int, required=True,
    ...
    parser.add_argument(
            '--k', type=int,
            help='Mask repeat parameter for k dropout')
    parser.add_argument(
            '--pool_size', type=int,
            help='Number of masks in the pool for pool k dropout')
    parser.add_argument(
            '--masks_per_batch', type=int,
            help='Number of submasks to include in each batch for k dropout')
    # dataset
    parser.add_argument(
            '--dataset_name', type=str, choices=DATASETS, required=True,
    ...
    parser.add_argument(
            '--preprocess_dataset', action='store_true',  # defaults to false
    ...
    # experiment
    parser.add_argument('--use_wandb', action='store_true',  # defaults to false
    ...
    dropout_layer, layer_kwargs = get_dropout_layer(
            dropout_layer=args.dropout_layer, p=args.p, k=args.k,
            pool_size=args.pool_size, masks_per_batch=args.masks_per_batch)
    model = make_net(
            input_dim=args.input_size, num_classes=args.output_size,
            hidden_units=args.hidden_size, hidden_layers=args.n_hidden,
            dropout_layer=dropout_layer, dropout_kargs=layer_kwargs)
    print(f'Created {args.dropout_layer} model with {args.n_hidden} hidden layers')
    print(model)

    # get dataset
    train_set, test_set = get_dataset(
            args.dataset_name, args.batch_size, args.test_batch_size,
            args.num_workers, args.preprocess_dataset, args.device)
    print(f'Loaded {args.dataset_name} dataset')

    # train
    print(f'Training for {args.epochs} epochs...')
    train_net(
            net=model, train_set=train_set, test_set=test_set, epochs=args.epochs,
            lr=args.lr, device=args.device, use_wandb=args.use_wandb)
